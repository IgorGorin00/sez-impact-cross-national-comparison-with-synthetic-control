...
def vizualize_synth(result: pd.DataFrame, region: str, variable: str, year: int):
    with plt.style.context('dark_background'):
        ...
        plt.vlines(x=year, ymin=0, 
                   ymax=result.Observed.max()*1.02 if result.Observed.max() > result.Synthetic.max() else result.Synthetic.max()*1.02,
...
def vizualize_post_pre(variable_dict: dict, variable_name: str, colors: list):
    with plt.style.context('dark_background'):
        ...
        plt.bar(x=variable_dict['x'], height=variable_dict['height'], color=colors)
